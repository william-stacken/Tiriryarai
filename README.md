# Tiriryarai
An HTTPS man-in-the-middle proxy written in C#.
...
In the [`TiriryaraiMitm`](TiriryaraiMitm) example plugin, they can be found in the application data
...
## 4. Adding Plugins
 ...
 - In the main method, create an instance of the [`HttpsMitmProxy`](Tiriryarai/Server/HttpsMitmProxy.cs) class
   and call its `Start()` method by passing an instance of the class implementeing `IManInTheMiddle`, your IP
   address, a port number, and a directory to keep the files mentioned in [2. Files](#2-files).
